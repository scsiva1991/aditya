...
var propTypes = {
  ...
  labelSelectedAnimatedColor: PropTypes.string,
...
var FloatingLabel  = React.createClass({
  ...
  getInitialState () {
    ...
    dirtyStyle.color = this.props.labelSelectedAnimatedColor;
    cleanStyle.color = this.props.labelPlaceholderColor;
    //alert(JSON.stringify(dirtyStyle));

    var style = state.dirty ? dirtyStyle : cleanStyle
    state.labelStyle = {
      fontSize: new Animated.Value(style.fontSize),
      top: new Animated.Value(style.top),
      color: new Animated.Value(style.color)
    }
  ...
  getDefaultProps: function() {
    return {
      labelSelectedAnimatedColor: '#9C389C',
      labelPlaceholderColor: 'grey',
    };
  },
  ...
  render() {
    var props = {
      ...
      if(this.props.multiline && this.props.multiline ==true){
          props.style.push(styles.inputMultiLine);
      }
